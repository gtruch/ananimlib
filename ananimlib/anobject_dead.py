...
class CompositeAnObject(AnObject):
    ...
    def __init__(self,anobjects=None,names=None):
        ...
        super().__init__(AnObjectContainer(anobjects,names),
                         al.CompositeRender())
    ...
    def add(self, anobject, key=None, path=None, update_transform=True):
        ...
        if path is None: 
            ...
            for anobject, key in zip(anobjects,keys):
                ...
                self.lookup[anobject] = key       # Reverse dictionary
                self.keys.append(key)             # Key list to maintain 
                                                  # insertion order
                
        elif len(path) > 1:
            self.get(path[0]).add(anobject, key, path[1:])
        else:
            self.add(anobject, key, path = None)
    ...
    def get(self,key):
        ...
        if isinstance(key,str):
            pass
        elif isinstance(key,al.AnObject):
            pass
        elif len(key) > 1: 
